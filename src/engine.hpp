...
public:
 ...
 void drawScene(const Scene&,const XMMATRIX& view,const XMMATRIX& proj);
 void endFrame();
 void setVsync(bool v){vsync_=v;}
 bool ready()const{return device_!=nullptr&&rtv_!=nullptr&&dsv_!=nullptr;}
 ID3D11Device* device()const{return device_.Get();}
private:
 bool makeShaders(); bool makeBuffers(); bool makeTargets(); void drawEntity(const Entity&,const XMMATRIX&,const XMMATRIX&);
 ComPtr<ID3D11Device> device_; ComPtr<ID3D11DeviceContext> context_; ComPtr<IDXGISwapChain> swap_; ComPtr<ID3D11RenderTargetView> rtv_; ComPtr<ID3D11DepthStencilView> dsv_; ComPtr<ID3D11Texture2D> depth_; ComPtr<ID3D11VertexShader> vs_; ComPtr<ID3D11PixelShader> ps_; ComPtr<ID3D11InputLayout> layout_; ComPtr<ID3D11Buffer> vb_; ComPtr<ID3D11Buffer> cb_; ComPtr<ID3D11RasterizerState> raster_;
...
private:
 void simulate(float dt); XMMATRIX cameraView()const; XMMATRIX cameraProjection()const;
 HWND input_{}; Graphics gfx_; Scene scene_; bool playing_=false; float time_=0; int width_=1,height_=1;
};
